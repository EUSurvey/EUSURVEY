...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	...
	<div style="background-color: #f5f5f5; position: absolute; left: 0; right: 0; margin-bottom: 20px;">
		<div style="width: 1206px; margin-left: auto; margin-right: auto; text-align: left;">
		  <a style="display: inline-block; color: #004494; font-weight: bold; padding: 12px; padding-left: 30px; padding-right: 30px; font-size: 16px" href="https://ec.europa.eu/commission/index_en">Commission and its priorities</a>
		  <a style="display: inline-block; background-color: #004494; color: #fff; font-weight: bold; padding: 12px; padding-left: 30px; padding-right: 30px; font-size: 16px" href="https://ec.europa.eu/info/index_en">Policies, information and services</a>
		</div>
	</div>
					...
					<c:when test="${form.getLanguagesAlphabetical().size() > 1}">
						<c:forEach var="lang" items="${form.getLanguagesAlphabetical()}">
							<c:if test="${lang.value.code == form.language.code}">
								<a onclick="$('#choseRunnerLanguageDialog').show()">${lang.value.name}</a>
								<a onclick="$('#choseRunnerLanguageDialog').show()">
	...
	<div id="choseRunnerLanguageDialog">
		...
		<div id="choseRunnerLanguageDialogInner">		
			...
			<div style="text-align: right; margin-right: 20px;">
				<a class="closedialog" onclick="$('#choseRunnerLanguageDialog').hide()">${form.getMessage("label.Close")}</a>
			</div>
