...
<body id="tosBody">
	...
	<form:form id="tos-form" action="${contextpath}/auth/tos" method="post"
		...
		<div <c:if test="${readonly == null}">class="pageheader"</c:if>>
			...
			<div id="tos_EN">
				<c:if test="${readonly == null}">
					<h1>Please read and accept our terms of use before using
						EUSurvey for the first time</h1>
				</c:if>
				...
				<c:if test="${readonly == null}">
					...
					<div style="text-align: center">
						...
						<div class="validation-error hideme" id="mustacceptEN">You
							have to accept the terms of use to be able to use EUSurvey</div>
						<br />
						<br /> <input type="submit" class="btn btn-primary"
							value="Submit" id="submitAcceptTosEN" /> <a
							href="${contextpath}/j_spring_security_logout"
			...
			<div id="tos_DE" style="display: none;">
				<c:if test="${readonly == null}">
					<h1>Bitte lesen und akzeptieren Sie unsere
						Nutzungsbedingungen, bevor Sie EUSurvey zum ersten Mal nutzen</h1>
				</c:if>

				<c:choose>
					<c:when test="${oss}">
						<%@ include file="tos_language/tos.oss_de.jsp"%></c:when>
					<c:otherwise>
						<%@ include file="tos_language/tos_de.jsp"%></c:otherwise>
				</c:choose>
				<c:if test="${readonly == null}">
					<br />
					<br />
			...
			<div id="tos_FR" style="display: none;">
				<c:if test="${readonly == null}">
					<h1>Veuillez lire et accepter nos conditions d'utilisation
						avant d'utiliser EUSurvey pour la première fois</h1>
				</c:if>
				<c:choose>
					<c:when test="${oss}">
						<%@ include file="tos_language/tos.oss_fr.jsp"%></c:when>
					<c:otherwise>
						<%@ include file="tos_language/tos_fr.jsp"%></c:otherwise>
				</c:choose>
				<c:if test="${readonly == null}">
					<br />
					<br />
