...
var closeOverlayDivsEnabled = false;
		...
		function deleteDelphiComment(button) {
			...
			const commentId = $(buttonParent).attr("data-id");
			const answerSetUniqueCode = $(buttonParent).attr("data-unique-code");
			...
			$.ajax({
				...
				success: () => {
					if ($(buttonParent).hasClass("reply")
						&& ($(buttonParent).prev().length === 0 || $(buttonParent).prev().hasClass('comment'))
						&& ($(buttonParent).next().length === 0 || $(buttonParent).next().hasClass('comment'))) {
						// As the Edit and Delete buttons are generated by the server and as a comment without replies
						// but with the text [DELETED] is treated as a special case, the page will be refreshed.
		...
		function editDelphiComment(button) {
			...
			const buttonParent = $(button).parent();
			const textElement = $(buttonParent).find('span');
			const text = textElement.text();
			const colonIndex = text.indexOf(':');
			const comment = text.substring(colonIndex + 2);
			const actionButtons = $(buttonParent).find('a');
			...
			$(div).append('<textarea class="form-control">' + comment + '</textarea>');
