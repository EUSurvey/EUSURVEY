...
$.widget('ui.spinner', {
	...
	_createButtons: function(input) {
		...
		wrapper.append(btnContainer.css({ height: height, left: -buttonWidth-rightMargin+5,
			// use offset calculation to fix vertical position in Firefox
			top: '-4px' }));
		//top: (input.offset().top - 4) + 'px' }));
		...
		icons.css({ marginLeft: "0px", marginTop:  (buttons.innerHeight() - icons.height()) / 2 });
	...
	_validate: function(value) {
		...
		if ((max != null) && (value > max))
			{
				$(this.element).closest("div").find(".limitnote").show();
				return max;
			}
