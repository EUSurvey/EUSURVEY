...
public class AnswerService extends BasicService {
	...
	public void internalSaveAnswerSet(AnswerSet answerSet, String fileDir, String draftid, boolean invalidateExportsAndStatistics, boolean createAttendees)  throws Exception
	{
		...
		if (answerSet.getSurvey().getShortname().equalsIgnoreCase("NewSelfRegistrationSurvey"))
		{
			...
			{
				if (element.getShortname().equalsIgnoreCase("name")) nameQuestion = element.getId(); else
					if (element.getShortname().equalsIgnoreCase("email")) {emailQuestion = element.getId(); emailElement = element;} else
						if (element.getShortname().equalsIgnoreCase("password")) passwordQuestion = element.getId(); else
							if (element.getShortname().equalsIgnoreCase("language")) {
			...
			if (createAttendees && answerSet.getSurvey().getRegistrationForm())
			{
				...
				for (Answer answer : answerSet.getAnswers()) {
					...
					{
						if (question.getAttributeName().equalsIgnoreCase("name")) attendee.setName(answer.getValue());
						else if (question.getAttributeName().equalsIgnoreCase("email")) attendee.setEmail(answer.getValue());
						else {
	...
	@Transactional(readOnly = true)
	public List<AnswerSet> getDraftAnswers(int surveyId, ResultFilter filter, SqlPagination sqlPagination, boolean loadDraftIds, boolean initFiles) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		...
		where.append("(ans.RESPONDER_EMAIL = :email OR RESPONDER_EMAIL = :email2)");
		...
		sql = "SELECT MIN(ans.ANSWER_SET_ID) FROM ANSWERS_SET ans " + joinSurveys + " WHERE " + where.toString() + " GROUP BY ans.UNIQUECODE HAVING min(ans.ISDRAFT) = 1 AND max(ans.ISDRAFT) = 1 ORDER BY ans.ANSWER_SET_UPDATE DESC";
		parameters.put("email", filter.getUser());		
		parameters.put("email2", Tools.md5hash(filter.getUser()));
	...
	@Transactional(readOnly = true)
	public List<AnswerSet> getAnswers(int surveyId, ResultFilter filter, SqlPagination sqlPagination, boolean loadDraftIds, boolean initFiles) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		HashMap<String, Object> parameters = new HashMap<>();
		...
		if (loadDraftIds)
		{
			sql = "select max(ans.ANSWER_SET_ID), max(d.DRAFT_UID), min(ans.ISDRAFT) from ANSWERS_SET ans LEFT JOIN ANSWERS_SET ans2 ON ans.UNIQUECODE = ans2.UNIQUECODE LEFT JOIN DRAFTS d ON ans2.ANSWER_SET_ID = d.answerSet_ANSWER_SET_ID LEFT JOIN SURVEYS s ON ans2.SURVEY_ID = s.SURVEY_ID where ans.ANSWER_SET_ID IN (" + getSql(null, surveyId, filter, parameters, true, true) + ") GROUP BY ans.UNIQUECODE";
		} else {
			sql = getSql(null, surveyId, filter, parameters, true, true);
		}
	...
	public List<String> deleteAnswers(int surveyId, ResultFilter filter) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		HashMap<String, Object> parameters = new HashMap<>();
	...
	@Transactional(readOnly = true)
	public Set<String> getCaseIds(Integer surveyId, ResultFilter filter, int page, int rowsPerPage, boolean searchallsurveys) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		HashMap<String, Object> parameters = new HashMap<>();
	...
	@Transactional(readOnly = true)
	public Set<Integer> getAllAnswerIds(Integer surveyId, ResultFilter filter, int page, int maxValue) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		HashMap<String, Object> parameters = new HashMap<>();
	...
	@Transactional
	public List<AnswerSet> getAllAnswers(int surveyId, ResultFilter filter) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		HashMap<String, Object> parameters = new HashMap<>();
	...
	@Transactional(readOnly = true)
	public List<File> getAllUploadedFiles(int surveyId, ResultFilter filter, int page, int rowsPerPage) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		HashMap<String, Object> parameters = new HashMap<>();
		...
		if (filter.isEmpty())
		{
			...
			sql = "SELECT f.FILE_ID, f.FILE_NAME, f.FILE_UID, af.ANSWERS_ANSWER_ID FROM FILES f JOIN ANSWERS_FILES af ON f.FILE_ID = af.files_FILE_ID JOIN ANSWERS a ON af.ANSWERS_ANSWER_ID = a.ANSWER_ID JOIN ANSWERS_SET ans ON a.AS_ID = ans.ANSWER_SET_ID where ans.ANSWER_SET_ID in (" + getSql(null, surveyId, filter, parameters, false, true) + ")";
		}
	...
	@SuppressWarnings("unchecked")
	@Transactional List<LiveStatistics> getLiveStatistics(Survey survey)
	{
	...
	@Transactional(readOnly = true)
	public boolean userAnsweredSurvey(Survey survey, User user) {
		Session session = sessionFactory.getCurrentSession();
	...
	@Transactional(readOnly = true)
	public int getNumberOfAnswerSets(Survey survey, ResultFilter filter) throws TooManyFiltersException {
		Session session = sessionFactory.getCurrentSession();
		
		if (!survey.getIsDraft() && filter.isEmpty()) return getNumberOfAnswerSetsPublished(survey.getShortname(), survey.getUniqueId());
...
//	@Transactional(readOnly = true)
//	public int getNumberOfContributions(String email) {
//		Session session = sessionFactory.getCurrentSession();
//		Query query = session.createQuery("SELECT count(*) FROM AnswerSet a WHERE a.survey.isDraft = false and a.responderEmail is not null and length(a.responderEmail) > 0 and a.responderEmail != 'd41d8cd98f00b204e9800998ecf8427e' and (a.responderEmail = :email OR a.responderEmail = :email2)").setString("email", email).setString("email2",Tools.md5hash(email));
//		long result = (Long) query.uniqueResult();
//		return (int)result;
//	}

//	@Transactional(readOnly = true)
//	public List<AnswerSet> getContributions(String email, int page, int rowsPerPage) {
//		Session session = sessionFactory.getCurrentSession();
//		Query query = session.createQuery("SELECT a FROM AnswerSet a WHERE a.survey.isDraft = false and a.responderEmail is not null and length(a.responderEmail) > 0 and a.responderEmail != 'd41d8cd98f00b204e9800998ecf8427e' and (a.responderEmail = :email OR a.responderEmail = :email2) ORDER BY a.updateDate DESC").setString("email", email).setString("email2",Tools.md5hash(email));
//		@SuppressWarnings("unchecked")
//		List<AnswerSet> result = query.setFirstResult((page - 1)*rowsPerPage).setMaxResults(rowsPerPage).list();
//		return result;
//	}
...
//		}
		...
		query.executeUpdate();
		//		@SuppressWarnings("unchecked")
//		List<ExportCache> list2 = query.list();
//		for (ExportCache c : list2) {
//			session.delete(c);
//		}
	}
	...
	public Statistics getStatisticsForFilterHash(int surveyId, String hash, boolean useEagerLoading)
	{
		...
		Query query = session.createQuery("SELECT s FROM Statistics s WHERE s.surveyId = :surveyId AND s.filterHash = :hash ORDER BY ACCESS_ID DESC").setInteger("surveyId", surveyId).setString("hash", hash);
		@SuppressWarnings("unchecked")
		List<Statistics> list = query.list();
		if (list.size() == 0)
		{
	...
	{
		Session session = sessionFactory.getCurrentSession();		
		Query query = session.createSQLQuery("select count(*) from ANSWERS a INNER JOIN ANSWERS_SET ans ON ans.ANSWER_SET_ID = a.AS_ID INNER JOIN SURVEYS s ON s.SURVEY_ID = ans.SURVEY_ID where s.ISDRAFT = :isdraft AND ans.UNIQUECODE != :ansuid AND ((a.QUESTION_ID= :questionId and a.VALUE= :value and a.ANSWER_ISDRAFT=0) or (a.QUESTION_UID= :questionUid and a.VALUE= :value and a.ANSWER_ISDRAFT=0))").setBoolean("isdraft", surveyIsDraft).setString("value", value).setString("ansuid", answerSetUniqueCode) .setString("questionUid", questionUid).setInteger("questionId", questionId);
		return ConversionTools.getValue(query.uniqueResult());
	}
	...
	public List<String> getQuestionsWithUploadedFiles(Survey survey) {
		...
		if (uids.size() == 0) return new ArrayList<>();
	...
	public int getNumberOfDrafts(int id) {
		...
		if (allVersions.size() == 0) return 0;
		String query = "SELECT COUNT(*) FROM ( SELECT * FROM ANSWERS_SET ans WHERE ans.SURVEY_ID IN (" + StringUtils.collectionToCommaDelimitedString(allVersions) + ") GROUP BY ans.UNIQUECODE HAVING MIN(ans.ISDRAFT) = 1	) as dummy";
