...
public class SurveyService extends BasicService {
	...
	private List<Object[]> loadSurveysfromDatabase(String sql, HashMap<String, Object> parameters, SqlPagination sqlPagination) {
		Session session = sessionFactory.getCurrentSession();
		SQLQuery query = session.createSQLQuery(sql);			
	...
	public List<Survey> getSurveysIncludingTranslationLanguages(SurveyFilter filter, SqlPagination sqlPagination, boolean addInvitedAndDrafts) {
	...
	private List<String> getCompletedTranslations(Survey survey) {
		return translationService.getTranslationsForSurvey(survey.getId(), false)
			.stream()
			.filter(Translations::getActive)
	...
	@Transactional(readOnly = true)
	public Survey getSurveyByShortname(String shortname, boolean isDraft, User u, HttpServletRequest request, boolean initElements, boolean checkNotArchived, boolean checkNotDeleted) throws InvalidURLException {
		...
		Survey survey = surveyService.getSurvey(shortname, isDraft, false, false, false, null, true, checkNotDeleted, true);
		...
		if (survey != null) {
			...
			if (u != null)
			{
				...
				if (allowed) sessionService.updateSessionInfo(survey, u, request);
			}
	...
	@Transactional
	public Survey getSurvey(String uidorshortname, boolean isDraft, boolean checkActive, boolean readReplies, boolean useEagerLoading, String language, boolean readonly) {
		return getSurvey(uidorshortname, isDraft, checkActive, readReplies, useEagerLoading, language, readonly, false, false);
	}
	
	@Transactional
	public Survey getSurvey(String uidorshortname, boolean isDraft, boolean checkActive, boolean readReplies, boolean useEagerLoading, String language, boolean readonly, boolean checkNotDeleted, boolean shortnamefirst) {
		Session session = sessionFactory.getCurrentSession();
		String sql;
		...
		if (id > 0) {
			...
			if (checkActive && !isDraft)
			{
				...
				if (!draft.getIsActive()) return null;
			}
	...
	public Survey add(Survey survey, boolean synchronize, int userId) {
		...
		if (synchronize) synchronizeTranslation(survey, userId);
		return result;
	}	
	...
	@Transactional
	public Survey activate(Survey survey, int pnumberOfAnswerSets, int pnumberOfAnswerSetsPublished, boolean deactivateAutoPublishing, int userId, boolean resetSourceIds, boolean resetSurvey) throws Exception
	{
		...
		{			
			if (!translation.getLanguage().getCode().equalsIgnoreCase(survey.getLanguage().getCode()))
			{
				...
				for (Translation t: translation.getTranslations())
				{
					...
					if (t.getKey().equalsIgnoreCase(Survey.TITLE) || t.getKey().equalsIgnoreCase(Survey.INTRODUCTION) || t.getKey().equalsIgnoreCase(Survey.ESCAPEPAGE) || t.getKey().equalsIgnoreCase(Survey.ESCAPELINK) || t.getKey().equalsIgnoreCase(Survey.CONFIRMATIONPAGE) || t.getKey().equalsIgnoreCase(Survey.CONFIRMATIONLINK) || t.getKey().equalsIgnoreCase(Survey.HELP) || t.getKey().equalsIgnoreCase(Survey.QUIZWELCOMEMESSAGE) || t.getKey().equalsIgnoreCase(Survey.QUIZRESULTSMESSAGE))
					{
						tc.setKey(t.getKey());
	...
	@Transactional
	public Survey publish(Survey survey, boolean deactivateAutoPublishing, int userId) throws Exception
	{
		//this means to create a copy of the survey and save it as non-draft survey
		Session session = sessionFactory.getCurrentSession();	
		
		Survey published = getSurvey(survey.getShortname(), false, false, false, false, null, false);
	...
	public Survey clearChanges(String shortname, int userId) throws Exception {
		...
		Survey survey = this.getSurvey(shortname, true, false, false, false, null, false);
		
		boolean active = survey.getIsActive();
		
		Survey publishedSurvey = this.getSurvey(shortname, false, false, false, false, null, true);
		
		// hibernate: transactional change of local variable
		Survey newDraft = activate(publishedSurvey, survey.getNumberOfAnswerSets(), survey.getNumberOfAnswerSetsPublished(), false, userId, true, false);
	...
	public void unpublish(Survey survey, boolean deactivateAutoPublishing, int userId)
	{
		...
		Survey publishedSurvey = this.getSurvey(survey.getShortname(), false, false, false, false, null, false);
		
		if (publishedSurvey == null)
		{
			publishedSurvey = this.getSurvey(survey.getUniqueId(), false, false, false, false, null, false);
		}
	...
	public int applyChanges(Survey survey, boolean deactivateAutoPublishing, int userId, boolean resetSurey) throws Exception
	{
		...
		Survey publishedSurvey = getSurvey(survey.getShortname(), false, false, false, true, null, true);
				
		Survey newPublishedSurvey = activate(survey, -1, -1, deactivateAutoPublishing, userId, false, false);
	...
	public Survey update(Survey survey, boolean hasPendingChanges, boolean synchronizeTranslations, boolean mergeFirst, int userId) {
		...
		if (synchronizeTranslations) synchronizeTranslation(survey, userId);
	...
	public boolean[] checkRegistrationFormElements(Survey survey)
	{
		...
		for (Element element: survey.getElements())
		{
			if (element instanceof Question)
			{
				...
				{					
					if (question.getAttributeName().equalsIgnoreCase("name")) result[0] = true;
					if (question.getAttributeName().equalsIgnoreCase("email")) result[1] = true;
	...
	private void deleteSurveyData(int id, boolean deleteAnswers, boolean deleteAccesses, String uid, boolean deleteLogs)
	{
		...
		//remove dependencies (this is only needed for older surveys that have zombie dependencies
	...
	@Transactional
	public void delete(int id, boolean deleteLogs, int userid) throws Exception {
		deleteNoTransaction(id, deleteLogs);
	}
	
	public void deleteNoTransaction(int id, boolean deleteLogs) throws Exception {
		Session session = sessionFactory.getCurrentSession();
	...
	public boolean getIsSurveyPublished(String shortname) {
		Survey s = this.getSurvey(shortname, false, false, false, false, null, true);
		return s != null;
	}
	...
	@Transactional
	private void importOldPublishedSurvey(Survey survey, User user, Map<String, String> oldToNewUniqueIds) throws Exception
	{
		...
		this.update(copyOld, false, true, true, user.getId());
	}
	...
	public int importSurvey(ImportResult result, User user, boolean isRestore) throws Exception {
		...
		try {
			...
			importSurveyData(result, user, true, copy, oldToNewUniqueIds);
			
			if (result.getActiveSurvey() != null)
			{		
				HashMap<Integer, Integer> oldSourceIdsById = getSourceIdsById(result.getActiveSurvey());
				for (int id : result.getOldSurveys().keySet()) {
					importOldPublishedSurvey(result.getOldSurveys().get(id), user, oldToNewUniqueIds);
				}
				...
				importSurveyData(result, user, false, copyActive, oldToNewUniqueIds);
			}
	...
	private void importSurveyData(ImportResult result, User user, boolean draft, Survey survey, Map<String, String> oldToNewUniqueIds) throws Exception
	{
		List<Translations> translations = null;
		List<List<AnswerSet>> answerSets = new ArrayList<>();
		Map<Integer,List<File>> files = null;
		
		if (draft)
		{
			translations = result.getTranslations();			
			if (result.getAnswerSets() != null)
			answerSets.add(result.getAnswerSets());
			files = result.getFiles();
		} else {
			translations = result.getActiveTranslations();
			if (result.getActiveAnswerSets() != null)
			answerSets = result.getActiveAnswerSets();
		...
		if (answerSets != null && answerSets.size() > 0)
		{
			...
			//for (List<AnswerSet> as: answerSets)
			{	
				List<AnswerSet> as = answerSets.remove(0);				
	...
	private String translateKey(String key, Map<Integer, Element> elementsBySourceId, Map<String, String> oldToNewUniqueIds) {
		...
		if (key.equalsIgnoreCase(Survey.TITLE)) return key;
		if (key.equalsIgnoreCase(Survey.INTRODUCTION)) return key;
		if (key.equalsIgnoreCase(Survey.ESCAPEPAGE)) return key;
		if (key.equalsIgnoreCase(Survey.ESCAPELINK)) return key;
		if (key.equalsIgnoreCase(Survey.CONFIRMATIONPAGE)) return key;
		if (key.equalsIgnoreCase(Survey.CONFIRMATIONLINK)) return key;
		...
		try {		
			if (key.endsWith("help"))
			{
			    ...
			    if (elementsBySourceId.containsKey(retVal)) return elementsBySourceId.get(retVal).getUniqueId() + "help";
			} else if (key.endsWith(NumberQuestion.UNIT))
			{
			    ...
			    retVal = Integer.parseInt(uid);
			    if (elementsBySourceId.containsKey(retVal)) return elementsBySourceId.get(retVal).getUniqueId() + NumberQuestion.UNIT;
			} else if (key.endsWith(Confirmation.LABEL))
			{
			    ...
			    retVal = Integer.parseInt(uid);
			    if (elementsBySourceId.containsKey(retVal)) return elementsBySourceId.get(retVal).getUniqueId() + Confirmation.LABEL;
			} else if (key.endsWith(Confirmation.TEXT))
			{
			    ...
			    retVal = Integer.parseInt(uid);
			    if (elementsBySourceId.containsKey(retVal)) return elementsBySourceId.get(retVal).getUniqueId() + Confirmation.TEXT;
			} else if (key.endsWith(Section.TABTITLE))
			{
			    ...
			    retVal = Integer.parseInt(uid);
			    if (elementsBySourceId.containsKey(retVal)) return elementsBySourceId.get(retVal).getUniqueId() + Section.TABTITLE;
			} else if (key.endsWith("shortname"))
			{
			    ...
			    if (elementsBySourceId.containsKey(retVal)) return elementsBySourceId.get(retVal).getUniqueId() + "shortname";
			} else if (oldToNewUniqueIds.containsKey(key))
			{
				return oldToNewUniqueIds.get(key);
			} else {				
			    retVal = Integer.parseInt(key);
			    if (elementsBySourceId.containsKey(retVal)) return elementsBySourceId.get(retVal).getUniqueId();
			}
	...
	public List<Survey> getSurveysToStart() {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("FROM Survey s WHERE s.isDraft = true AND s.start <= :start AND s.end > :start AND s.automaticPublishing = true AND s.isActive = false").setTimestamp("start", new Date());
		return query.list();
	}
	...
	public Map<Element, Integer> getPendingChanges(Survey draft) {
		...
		Survey published = getSurvey(draft.getShortname(), false, false, false, false, null, true);
		...
		if (draft != null && published != null)
		{
			if (draft.getContact() != null && !draft.getContact().equals(published.getContact())) hasPendingChanges = true;
			if (draft.getMultiPaging() != published.getMultiPaging()) hasPendingChanges = true;
			if (draft.getValidatedPerPage() != published.getValidatedPerPage()) hasPendingChanges = true;
			if (!Objects.equals(draft.getWcagCompliance(), published.getWcagCompliance())) hasPendingChanges = true;
			if (!Tools.isFileEqual(draft.getLogo(),published.getLogo())) hasPendingChanges = true;
			if (!draft.getLogoInInfo().equals(published.getLogoInInfo())) hasPendingChanges = true;
			if (!Tools.isEqual(draft.getSkin(),published.getSkin())) hasPendingChanges = true;
			if (draft.getSectionNumbering() != published.getSectionNumbering()) hasPendingChanges = true;
			if (draft.getQuestionNumbering() != published.getQuestionNumbering()) hasPendingChanges = true;
			if (!Tools.isEqual(draft.getConfirmationPage(),published.getConfirmationPage())) hasPendingChanges = true;
			if (!Tools.isEqual(draft.getConfirmationPageLink(),published.getConfirmationPageLink())) hasPendingChanges = true;
			if (!Tools.isEqual(draft.getConfirmationLink(),published.getConfirmationLink())) hasPendingChanges = true;
		
			if (!Tools.isEqual(draft.getIsQuiz(),published.getIsQuiz())) hasPendingChanges = true;
			if (!Tools.isEqualIgnoreEmptyString(draft.getQuizWelcomeMessage(),published.getQuizWelcomeMessage())) hasPendingChanges = true;
			if (!Tools.isEqualIgnoreEmptyString(draft.getQuizResultsMessage(),published.getQuizResultsMessage())) hasPendingChanges = true;
			
			if (!Tools.isEqual(draft.getShowTotalScore(),published.getShowTotalScore())) hasPendingChanges = true;
			if (!Tools.isEqual(draft.getScoresByQuestion(),published.getScoresByQuestion())) hasPendingChanges = true;
			
			if (!Tools.isEqual(draft.getShowQuizIcons(),published.getShowQuizIcons())) hasPendingChanges = true;
			...
			if (hasPendingChanges) result.put(new PropertiesElement(), 1);
			...
			{
				hasPendingChanges = true;
				if (hasPendingChanges) result.put(new PropertiesElement(true), 1);
			}
			...
			if (hasPendingTranslationChanges) result.put(new TranslationsElement(), 1);
	...
	public Element getParentForChildQuestion(Integer id) {
		...
		//questions could have been deleted from the parent -> get newest version of the parent
		if (parentid > 0)
		{
	...
	{
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createSQLQuery("SELECT DISTINCT QUESTION_UID, PA_UID FROM ANSWERS a INNER JOIN ANSWERS_SET ans ON ans.ANSWER_SET_ID = a.AS_ID JOIN SURVEYS s ON  ans.SURVEY_ID =  s.SURVEY_ID WHERE s.ISDRAFT = FALSE AND s.SURVEY_UID = :surveyUid AND ans.ISDRAFT = false").setString("surveyUid", surveyUid);
	...
	{			
    	...
    	for (Object o: res)
    	{
    		...
    		if (!surveyelementsbyuid.containsKey(questionUID) && !missingelementuids.containsKey(questionUID))
			{
				...
				if (question == null && missingelementuids.containsKey(questionUID)) question = missingelementuids.get(questionUID);
	...
	@Transactional
	public boolean changeOwner(String surveyuid, int ownerid, int currentuserid)
	{
		User newowner = administrationService.getUser(ownerid);
		...
		if (newowner != null)
		{
			...
			if (survey != null & survey.getOwner().getId() != newowner.getId())
			{
	...
	public List<Integer> getAllPublishedSurveyVersions(int surveyId) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createSQLQuery("SELECT s1.SURVEY_ID FROM SURVEYS s1 JOIN SURVEYS s2 ON s1.SURVEY_UID = s2.SURVEY_UID WHERE s2.SURVEY_ID = :id AND s1.ISDRAFT=0").setInteger("id", surveyId);
	...
	public void sendListFormMail(Survey survey) throws NumberFormatException, IOException {
	...
	public void sendOPCApplyChangesMail(Survey survey, int userId) throws IOException
	{
		...
		{			
			...
			for (String opcuser : users) {
				if (opcuser.length() > 0)
				{
					if (opcuser != null)
					{
						mailService.SendHtmlMail(opcuser, sender, sender, "The online version of a published 'BRP Public Consultation' has been changed", text, smtpServer, Integer.parseInt(smtpPort), null);
					}
				}
			}
		}	
	...
	public Survey getSurveyForUploadedFile(int fileid) {
		...
		return getSurvey(result, true, false, false, false, null, true);
	}
	...
	@Transactional(readOnly = true)
	public String[] getSurveyForFile(File file, String contextpath) {
		Session session = sessionFactory.getCurrentSession();
	...
	public List<Element> getElements(String[] ids) {
		Session session = sessionFactory.getCurrentSession();
//		Query query = session.createQuery("FROM Element where ID IN (:ids)").setParameterList("ids", ids);
//		return query.list();
	...
	@Transactional(readOnly = true)
	public List<Survey> getDeletedSurveys(DeletedSurveysFilter filter, int page, int rowsPerPage) {
	...
	public List<String> getAllPublishedSurveysUIDsForUser(User user, boolean escapeforsql, boolean includearchived) {
		Session session = sessionFactory.getCurrentSession();		
		String sql = "SELECT DISTINCT s.SURVEY_UID from SURVEYS s LEFT JOIN MV_SURVEYS_NUMBERPUBLISHEDANSWERS npa on s.SURVEY_UID = npa.SURVEYUID where s.ISDRAFT = 1 and (s.OWNER = :userid OR s.SURVEY_ID in (Select a.SURVEY FROM SURACCESS a WHERE (a.ACCESS_USER = :userid OR a.ACCESS_DEPARTMENT IN (SELECT GRPS FROM ECASGROUPS WHERE eg_ID = (SELECT USER_ID FROM ECASUSERS WHERE USER_LOGIN = :login))) AND (a.ACCESS_PRIVILEGES like '%2%' or a.ACCESS_PRIVILEGES like '%1%'))) and (s.DELETED = 0 or s.DELETED is null)";
	...
	public String[] getMetaDataForUser(User user)
	{
		...
		{	
			...
			//this means that the user has no "exiting" survey. Check if there is an archived one
	...
	@Transactional(readOnly = true)
	public List<String> getAllSurveyUIDs() {
		Session session = sessionFactory.getCurrentSession();
