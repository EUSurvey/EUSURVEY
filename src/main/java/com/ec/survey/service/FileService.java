...
public class FileService extends BasicService {
	...
	@Transactional(readOnly = true)
	public List<File> getAll(String uid) throws FileNotFoundException {
	...
	public Map<String, String> getFileNamesForBackgroundDocuments(Map<String, String> backgroundDocuments) {
		...
		try {
			...
			{
				String fileUID = getFileUIDFromUrl(backgroundDocuments.get(key));
				File f = get(fileUID);
				...
				try {
					Files.walkFileTree(dir, new SimpleFileVisitor<Path>() {
						...
						public FileVisitResult preVisitDirectory(Path file, BasicFileAttributes attrs) throws IOException
						{
				    		...
				    		{
				    			return FileVisitResult.CONTINUE;
				    		} else if (filter.isTemporaryFiles() && !file.equals(archivedir) && !file.equals(filedir))
							{
				    			return FileVisitResult.CONTINUE;
							} else {
								return FileVisitResult.SKIP_SUBTREE;
	...
	private boolean checkResult(FileResult fileResult, FileFilter filter) {
		...
		if (filter.getFileTypes() != null && filter.getFileTypes().size() > 0)
		{
			boolean found = false;
		...
		if (filter.getFileExtensions() != null && filter.getFileExtensions().size() > 0)
		{
			boolean found = false;
			for (String ext : filter.getFileExtensions())
			{
				if (ext.equals(fileResult.getFileExtension()))
				{
					found = true;
					break;
				} else if (ext.equalsIgnoreCase("OTHER") && !Arrays.asList(fileextensions).contains(fileResult.getFileExtension()))
				{
					found = true;
					break;
				}
		...
		if (filter.isOnlyUnreferenced())
		{
			if (fileResult.getSurveyShortname() != null && fileResult.getSurveyShortname().length() > 0)
			{
				return false;
			}			
		}
		
		if (filter.getFilterApplied() != null)
		{
			if (fileResult.getFilterApplied() == null || !filter.getFilterApplied().equalsIgnoreCase(fileResult.getFilterApplied()))
			{
				return false;
			}
		}
		...
		for (int id : ids)
		{
			...
			if (filter.isSystemExports())
			{
				...
				for (Export export: exports)
				{
					...
					if (!skipexport)
					{
						...
						{
							if (filter.isValidExtension(export.getFormat().toString().toUpperCase()))
							{
			...
			if (filter.isVisible("contribution") && filter.isValidExtension("PDF"))
			{
				...
				while (!stop)
				{
					...
					if (caseids != null && caseids.size() < 1000) stop = true;
	...
	public FileResult getFileResult(Path file, Export export, Path archivedir) throws IOException {
		...
		if (name.startsWith("answer") && name.endsWith(".pdf"))
		{
			...
			try {
				if (export == null) export = exportService.get(Integer.parseInt(exportid), false);
				if (export != null)
				{
		...
		if (!onlySurveyFiles)
		{
			...
			for (int surveyID : surveyIDs)
			{
				...
				while (!stop)
				{
					...
					if (caseids.size() < 1000) stop = true;
		...
		while (!stop)
		{
			...
			for (FileResult result: results)
			{
				...
				if (file.delete())
				{
					...
					throw new Exception("not possible to delete file " + result.getFilePath());
				}
			}
		}
	...
	public int deleteContributions(Date pdfbefore) throws Exception {
				...
				if (file.exists())
				{
					...
					if (modified.before(pdfbefore))
					{
						if (file.delete())
						{
							...
							throw new Exception("not possible to delete file " + file.getAbsolutePath());
						}
					}
				}
			}
	...
	public int deleteTemporaryFiles(final Date tempbefore) throws Exception {
		logger.info("starting deleteContributions: " + ConversionTools.getFullString(new Date()));
		...
		Files.walkFileTree(dir, new SimpleFileVisitor<Path>() {
		    ...
		    public FileVisitResult visitFile(Path p, BasicFileAttributes attrs) throws IOException {					    	
		    	...
		    	{					    	
		    		if (!p.equals(dir) && !p.equals(filedir) && !p.equals(archivedir))
		    		{
	...
	public void createDummyFiles(int files, String filetype) throws IOException {
	...
	public boolean recreate(java.io.File file, java.io.File archivedir, Locale locale, MessageSource resources) throws IOException {
	...
	public File copyFile(String uid, String surveyUID) throws IOException, ForbiddenURLException {
	...
	public java.io.File getSurveyExportsFolder(String surveyUID, boolean create)
	{
        ...
        if (!folder.exists() && create) folder.mkdirs();
		return folder;
	}
	...
	public java.io.File getUsersFolder(int userId, boolean create)
	{
		...
		if (userfolderNumber > 0) userfolder += userfolderNumber;
				
		java.io.File folder = new java.io.File(usersDir + userfolder + "/" + userId + "/");
		
		if (!folder.exists() && !create) return null;
	...
	public void migrateSurveyFiles(Survey survey) throws Exception
	{
		...
		for (Element element : survey.getElements()) {
			if (element instanceof Image)
			{
				...
				if (image.getUrl() != null && image.getUrl().length() > 0)
				{
					...
					{
						image.setUrl(servletContext.getContextPath() + "/files/" + survey.getUniqueId() + "/" + fileUID);
					}
				}
		...
		for (String label : survey.getBackgroundDocuments().keySet())
		{
			...
			if (migrateSurveyFile(survey.getUniqueId(), fileUID))
			{
				survey.getBackgroundDocuments().put(label, sessionService.getContextPath() + "/files/" + survey.getUniqueId() + "/" + fileUID);
			}
		}
	...
	public void migrateAllArchiveFiles() throws Exception
	{
		...
		for (Archive archive : archives)
		{
			...
			if (original.exists())
			{
				java.io.File copy = new java.io.File(folder.getPath() + "/" + archive.getSurveyUID() + "statistics.pdf");
				Files.copy(original.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
			}
			...
			if (original.exists())
			{
				java.io.File copy = new java.io.File(folder.getPath() + "/" + archive.getSurveyUID() + "statistics.xls");
				Files.copy(original.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
			}
			...
			if (original.exists())
			{
				java.io.File copy = new java.io.File(folder.getPath() + "/" + archive.getSurveyUID() + "results.xls.zip");
				Files.copy(original.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
			}
		}
	}
