...
public class MultipleChoiceQuestion extends ChoiceQuestion {
	...
	public boolean differsFrom(Element element) {
		...
		if (!(element instanceof MultipleChoiceQuestion)) return true;
		
		MultipleChoiceQuestion multi = (MultipleChoiceQuestion)element;

		if (!(numColumns == multi.numColumns)) return true;
		if (!(maxChoices == multi.maxChoices)) return true;
		if (!(minChoices == multi.minChoices)) return true;
		if (!(useCheckboxes == multi.useCheckboxes)) return true;
		
		if (!Objects.equals(getOrder(), multi.getOrder())) return true;
		
		if (getPossibleAnswers().size() != multi.getPossibleAnswers().size()) return true;
		
		for (int i = 0; i < getPossibleAnswers().size(); i++)
		{
			if (!getPossibleAnswers().get(i).getTitle().equals(multi.getPossibleAnswers().get(i).getTitle())) return true;
			if (!getPossibleAnswers().get(i).getDependentElementsUIDString().equals(multi.getPossibleAnswers().get(i).getDependentElementsUIDString())) return true;
			if (!Tools.isEqual(getPossibleAnswers().get(i).getShortname(),multi.getPossibleAnswers().get(i).getShortname())) return true;
			if (getPossibleAnswers().get(i).getScoring() != null)
			{
				if (getPossibleAnswers().get(i).getScoring().differsFrom(multi.getPossibleAnswers().get(i).getScoring()))
				{
					return true;
				}
			}
