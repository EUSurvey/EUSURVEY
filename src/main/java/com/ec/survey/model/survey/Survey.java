 ...
 * It contains a list of elements that represent the different
 * form elements, like questions or sections.
 */
@Entity
...
public class Survey implements java.io.Serializable {
	...
	private boolean isPublished;   // true if there is a draft and a survey object, isPublished is only available in the draft object
	...
	private String escapePage = ESCAPETEXT; //"This survey has not yet been published or has already been unpublished in the meantime.";
	...
	{
		return link == null ? null : link.replace("{ALIAS}", shortname.toLowerCase()).replace("{LANGUAGE}", language.toLowerCase());
	}	
	...
	public File getLogo() {return logo;}  
	public void setLogo(File logo) {this.logo = logo;}  
	...
	public Map<Integer, Element> getMissingElementsById() {
		...
		if (missingElementsById != null) return missingElementsById;
	...
	public Map<String, Element> getMissingElementsByUniqueId() {
		...
		if (missingElementsByUniqueId != null) return missingElementsByUniqueId;
	...
	public String getNotificationValue() {
		if (!automaticPublishing) return null;
	...
	{
		if (getNotificationUnit() == null) return "";
	...
	protected static Comparator<Element> newElementByPositionComparator() {
		 return (first, second) -> {
             ...
             if (result == 0) result = first.getId().compareTo(second.getId());
	...
	public String serialize(boolean elementOrderOnly) {
		...
		if (!elementOrderOnly)
		{
			...
			try {
				...
				{
					result.append(" BackgroundDocument: ").append(key).append(" - ").append(backgroundDocuments.get(key)).append(";");
				}
			}
			catch (Exception e)
			{}
			...
			try {
				...
				for (String key: usefulLinks.keySet())
				{
					result.append(" UsefulLink: ").append(key).append(" - ").append(usefulLinks.get(key)).append(";");
				}
			}
			catch (Exception e)
			{}
	...
	@Transient
	public Survey copy(SurveyService surveyService, User powner, String fileDir, boolean copyNumberOfAnswerSets, int pnumberOfAnswerSets, int pnumberOfAnswerSetsPublished, boolean copyPublication, boolean resetSourceIds, boolean keepuid, String newshortname, String newuid) throws Exception
	{
		...
		if (resetSourceIds)
		{
			...
			//we need to set the source id of the elements in the original to that of the copy (the new draft)
	...
	@Transient
	public Map<Integer, Element> copyElements(Survey copy, SurveyService surveyService, boolean makeQuestionsLocked) throws IntrusionException, ValidationException
	{
		for (Element element : elements) {
