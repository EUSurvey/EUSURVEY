...
/**
 * Represents a privilege that a user (not the owner) has for
 * a survey
 */
@Entity
...
public class Access {
	...
	public String getPrivileges() {
		...
		for (LocalPrivilege privilege : localPrivileges.keySet()) {
			result.append(privilege.toString()).append(":").append(localPrivileges.get(privilege)).append(";");
		}
	...
	public boolean hasAnyPrivileges() {
		if (localPrivileges.get(LocalPrivilege.AccessDraft) > 0) return true;
		if (localPrivileges.get(LocalPrivilege.AccessResults) > 0) return true;
		if (localPrivileges.get(LocalPrivilege.FormManagement) > 0) return true;
		if (localPrivileges.get(LocalPrivilege.ManageInvitations) > 0) return true;
