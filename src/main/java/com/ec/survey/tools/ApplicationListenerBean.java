...
public class ApplicationListenerBean implements ApplicationListener<ContextRefreshedEvent> {
	...
	private static void updateSchema(SchemaService schemaService, ServletContext servletContext) throws IOException {
		...
		if (status.getDbversion() < 136){
			logger.info("starting upgrade step 136");
			schemaService.step136();
			status = schemaService.getStatus();
		}

		if (status.getDbversion() < 137){
			logger.info("starting upgrade step 137");
			schemaService.step137();
			status = schemaService.getStatus();
		}

		if (status.getDbversion() < 138){
			logger.info("starting upgrade step 138");
			schemaService.step138();
			status = schemaService.getStatus();
		}

		if (status.getDbversion() < 139){
			logger.info("starting upgrade step 139");
			schemaService.step139();
			status = schemaService.getStatus();
		}

		if (status.getDbversion() < 140){
			logger.info("starting upgrade step 140");
			schemaService.step140();
			status = schemaService.getStatus();
		}

		if (status.getDbversion() < 141){
			logger.info("starting upgrade step 141");
			schemaService.step141();
			status = schemaService.getStatus();
		}

		if (status.getDbversion() < 142){
			logger.info("starting upgrade step 142");
			schemaService.step142();
			status = schemaService.getStatus();
		}
	}
